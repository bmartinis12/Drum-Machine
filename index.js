  ...
  return (
    <div className="bg-info min-vh-100 text-black" id="drum-machine">
      <div className="text-center" id="display">
        <h2>Drums</h2>
        <div className="drum-pad" id="letter">
        {audioClips.map((clip) => (
          <Pad className="clip" key={clip.id} clip={clip} volume={volume} setRecording={setRecording} />
        ))}
        </div>
        <br />
        <h4>Volume</h4>
...
function Pad({ clip, volume, setRecording }){
  ...
  return (
    <div onClick={playSound} className={`btn btn-secondary p-4 m-3 drum-pad ${active && "btn-warning"}`}>
